...
public interface AccountRepository extends JpaRepository<Account, Long> {
    ...
    List<Account> findAllByCreateDate(LocalDate date, Long id);

    // 금일 지출 금액 합계 조회
    @Query("SELECT SUM(a.amount) FROM Account a WHERE a.createDate= :date AND a.accountBook.id = :id ")
    Integer sumTodayExpenses(LocalDate date, Long id);
    ...
    Integer sumExpensesByMonth(int year, int month, Long id);
    ...
    List<Object[]> countExpensesByCategoryForMonth(int year, int month, Long id);
    ...
    List<Object[]> getExpensesByCategory(int year, int month, Long id);
    ...
    List<Object[]> getExpensesBySatisfaction(int year, int month, Long id);
    ...
    List<Object[]> sumExpensesByDayForMonth(int year, int month, Long id);
